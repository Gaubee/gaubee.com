...
/**参考 {@link Pluggable} */
type SafePluggable<Params extends unknown[], Input extends import('unist').Node, Output extends import('unist').Node = Input> =
  | SafePlugin<Params, Input, Output>
  | SafePluginTuple<Params, Input, Output>;
type SafePlugin<Params extends unknown[], Input extends import('unist').Node, Output extends import('unist').Node> = (
  this: Processor,
  ...parameters: Params
) => void | import('unified').Transformer<Input, Output>;
type SafePluginTuple<Params extends unknown[], Input extends import('unist').Node, Output extends import('unist').Node> = [SafePlugin<Params, Input, Output>, ...Params];
...
const remarkPlugins = [markdownMermaidPlugins.remarkPlugins, markdownHightlightPlugins.remarkPlugins].flat().filter((it) => it !== undefined);
const rehypePlugins = [markdownMermaidPlugins.rehypePlugins, markdownHightlightPlugins.rehypePlugins].flat().filter((it) => it !== undefined);
...
export const markdownToHtml = async (content: string, opts: {filepath: string}) => {
  ...
  if (opts.filepath.endsWith('.mdx')) {
    ...
    const runtime = (await import(isDev ? 'react/jsx-dev-runtime' : 'react/jsx-runtime')) as typeof import('react/jsx-dev-runtime') | typeof import('react/jsx-runtime');
