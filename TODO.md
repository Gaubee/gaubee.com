...
## oxfmt 格式化工具链 + 视图异步加载 + 状态栏启动进度条（2026-07-24）

### oxfmt 格式化（替代 prettier）

- 安装 oxfmt@0.60.0（Rust 实现，Prettier-compatible，内置 import 排序），移除 prettier + prettier-plugin-organize-imports。
- `.oxfmtrc.json`：开启 sortImports，排除 build/、.svelte-kit/、生成产物。
- `pnpm fmt` / `pnpm fmt:check` 脚本（兑现 agents.md 约定）。
- 全量格式化 284 文件（统一缩进/import 排序）。

### 视图异步加载（首屏 bundle 优化）

- registry.ts：ViewEntry.component → loader（ViewLoader 懒加载工厂）。
- placeholders.ts：15 个静态 import → 动态 `() => import(...)`，与 manifest 的 activity.view 同源。
- AreaOutlet.svelte：$effect 触发 loader 异步加载 + loadedSlots 缓存保活（加载后常驻 DOM，编辑器/终端会话不丢）+ 骨架态。
- deep link / pop view 同样异步加载。

### 状态栏启动进度条

- app-load.svelte.ts：AppLoadStore（start/done/isLoading），独立于 navStore 的纯函数 reducer 模型。
- SystemStatusBar.svelte：header 加 relative，底边内嵌 .app-loading-bar（absolute 2px 细线，var(--primary) 主题色，indeterminate 滑动动画，不占布局高度）。
- prefers-reduced-motion 适配。

### 验证

- `pnpm fmt`（284 文件）、`pnpm check`（0 错误）、`pnpm test:unit`（238 全过）、`pnpm build` 成功。
- Playwright E2E：进度条 CSS 规则（absolute/2px/background）+ header 高度不变 + 慢速网络下进度条 108 次出现确认链路生效。

## 主题应用（自定义 primary 色相 + 桌面背景）（2026-07-24）
