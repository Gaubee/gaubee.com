...
- [x] **优化响应式布局**:
  ...
  - [x] 移动设备状态下（就是说右边部分无法显示的情况下），顶部应该出现一个topbar，右边部分用抽屉的模式打开。
...
## 第五阶段：跨平台 Scaffold 组件 (已完成)

- [x] 设计并实现跨平台 Scaffold 组件，支持 Mobile/Tablet/Desktop 响应式布局
  - [x] 实现 AppBar 组件（含沉浸式、滚动响应、折叠）
  - [x] 实现 Drawer 组件（左侧/右侧，手势返回支持）
  - [x] 实现 BottomNavigationBar 组件（自动隐藏滚动）
  - [x] 实现 NavigationRail 组件（可折叠）
  - [x] 实现 SettingPanel 组件（全局设置面板）
  - [x] 实现响应式布局，在不同屏幕尺寸时自动切换不同的版面
  - [x] 支持 SSR 水合
- [x] 创建完整的文档和使用示例