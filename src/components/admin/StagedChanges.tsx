...
const statusStyles: { [key in StagedChange['status']]: 'default' | 'destructive' | 'outline' } = {
  created: 'default',
  updated: 'outline',
  deleted: 'destructive',
};
...
const DiffDialog = ({ change }: { change: StagedChange }) => {
    const handleUnstage = async () => {
        if (window.confirm(`Are you sure you want to unstage changes for "${change.path}"?`)) {
    ...
    return (
        <DialogContent className="max-w-4xl">
            <DialogHeader>
                <DialogTitle>Diff for {change.path.replace('src/content/', '')}</DialogTitle>
                <DialogDescription>
    ...
    <Card>
      ...
      <CardContent>
        ...
        {!loading && changes.length === 0 && (
          <p className="text-muted-foreground text-center p-4">No changes have been staged.</p>
        )}
        {!loading && changes.length > 0 && (
                ...
                <Dialog key={change.path}>
                    <DialogTrigger asChild>
                        <li className="flex justify-between items-center py-2 px-3 bg-accent rounded-md cursor-pointer hover:ring-2 ring-primary">
                            <span className="font-mono text-sm truncate">{change.path.replace('src/content/', '')}</span>
      ...
      {changes.length > 0 && (
        <CardFooter className="flex flex-col items-start gap-4">
            ...
            <Button
              ...
              disabled={isCommitting || changes.length === 0 || !commitMessage.trim()}
              className="w-full"
            >
