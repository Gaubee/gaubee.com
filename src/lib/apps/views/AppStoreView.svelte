<!--
	AppStoreView：应用市场（应用安装/卸载/详情）。
	根据 location.pathname 二级分发：/app/store → 列表，/app/store/{appId} → 详情。
-->
<script lang="ts">
  ...
  import AppStoreDetailView from './AppStoreDetailView.svelte'

  const navState = $derived(navStore.current)

  // pathname 二级分发：/app/store/{appId} → 详情，否则 → 列表
  const detailAppId = $derived.by(() => {
    const match = navState.mainLocation.pathname.match(/^\/app\/store\/(.+)$/)
...
{#if detailAppId}
  <!-- 详情页 -->
  <AppStoreDetailView appId={detailAppId} />
{:else}
  <!-- 列表 -->
