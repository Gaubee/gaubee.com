 ...
 */
export function parseBrowserLocation(loc: Location, layout: NavLayout): {
...
function locationHrefChanged(
  ...
  const prevLoc = area === 'main' ? prev.mainLocation : area === 'bottom' ? prev.bottomLocation : prev.popLocation
  const nextLoc = area === 'main' ? next.mainLocation : area === 'bottom' ? next.bottomLocation : next.popLocation
...
export class NavController {
  ...
  init(): void {
    ...
    this.state = applyBehaviorPlugins(this.state, this.state, { type: 'BOOTSTRAP' })
