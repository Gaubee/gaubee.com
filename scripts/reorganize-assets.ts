...
async function processMarkdownFile(filePath: string) {
  ...
  while ((match = imgRegex.exec(fileContent)) !== null) {
    ...
    const relativeImgPath = originalImgPath.startsWith('/') ? originalImgPath.substring(1) : originalImgPath;
    ...
    if (existsSync(sourceImgPath) && sourceImgPath.startsWith(oldImgDir)) {
        ...
        try {
            ...
            const newRelativePath = path.relative(path.dirname(filePath), destImgPath);
            newContent = newContent.replace(originalImgPath, newRelativePath.replace(/\\/g, '/'));
